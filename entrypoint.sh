...
case "$SERVICE" in
  ...
  web)
    ...
    exec poetry run gunicorn "web_main:app" --bind 0.0.0.0 --workers 2 --reload --log-level \
      info --timeout 120 --access-logfile - --error-logfile - --forwarded-allow-ips=172.23.0.6
    ;;
